...
def main():
    csv_path = "/home/wam/grad/s14-range-delete-study/results/summary/ls24-density-preserved.csv"
    out_svg = "/home/wam/grad/s14-range-delete-study/results/plots/ls24-density-preserved/ls24_density_preserved_tradeoff.svg"
    ...
    svg.append('<text x="50" y="40" class="title">LS24-DensityPreserved: 24GiB (40% Tombstone Coverage) Tradeoff</text>')
    svg.append('<text x="50" y="62" class="subtitle">Preserved 40% Deletion Density on 24GiB Dataset: T=0 vs T=256 vs T=2048</text>')
    ...
    for i, (lab, val) in enumerate(zip(labels, put_p99s)):
        ...
        by = p2_y + 210 - bh
        color = "#ef4444" if val > 1500 else "#6366f1"
        svg.append(f'<rect x="{bx}" y="{by}" width="70" height="{bh}" fill="{color}" rx="4" />')
    ...
    with open(out_svg, 'w', encoding='utf-8') as f:
        f.write('\n'.join(svg))
    print(f"Saved LS24-DensityPreserved SVG plot to {out_svg}")
