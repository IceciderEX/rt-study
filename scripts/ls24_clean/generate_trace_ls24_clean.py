...
def main():
    src_trace = "/home/wam/grad/s14-range-delete-study/traces/ls24/ls24_dynamic_phases.bin"
    out_dir = "/home/wam/grad/s14-range-delete-study/traces/ls24_clean"
    os.makedirs(out_dir, exist_ok=True)
    out_trace = os.path.join(out_dir, "ls24_clean_phases.bin")

    if not os.path.exists(src_trace):
        print(f"Error: {src_trace} not found")
        return

    print("Generating LS24-Clean trace (replacing DeleteRange op_type=3 with No-op op_type=4)...")
    count_ops = 0
    count_noop = 0
