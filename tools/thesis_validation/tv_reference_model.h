...
class TVReferenceModel {
public:
    TVReferenceModel(uint64_t total_keys, size_t val_size)
        : total_keys_(total_keys), val_size_(val_size),
          is_alive_(total_keys, true),
          affected_live_flags_(total_keys, 0),
          live_keys_count_(total_keys),
          tombstones_count_(0) {}
    ...
    TVGetCategory ClassifyGet(uint64_t key_id) const {
    ...
    uint64_t GetLiveKeyCount() const {
        std::shared_lock<std::shared_mutex> lock(mutex_);
        return live_keys_count_;
    }

    uint64_t GetUnionDeletedKeys() const {
        std::shared_lock<std::shared_mutex> lock(mutex_);
        return total_keys_ - live_keys_count_;
    }

    double GetUnionCoverageRatio() const {
        std::shared_lock<std::shared_mutex> lock(mutex_);
        if (total_keys_ == 0) return 0.0;
        return static_cast<double>(total_keys_ - live_keys_count_) / total_keys_;
    }
    ...
    bool FullScanAndComputeSha256(rocksdb::DB* db, uint64_t& db_live_keys, uint64_t& db_data_bytes,
        ...
        while (it->Valid()) {
            ...
            if (k_slice.size() < 16 || std::memcmp(k_slice.data(), "key_", 4) != 0) {
                err_msg = "Full Scan Invalid Key format";
                return false;
            }

            uint64_t k_id = 0;
            const char* p = k_slice.data() + 4;
            const char* p_end = k_slice.data() + k_slice.size();
            while (p < p_end) {
                if (*p < '0' || *p > '9') {
                    err_msg = "Full Scan Invalid Key digits";
                    return false;
                }
                k_id = k_id * 10 + (*p - '0');
                p++;
            ...
            if (k_id >= total_keys_ || !is_alive_[k_id]) {
                err_msg = "Full Scan Found Deleted Key in DB: key_" + std::to_string(k_id);
                return false;
            }

            db_live_keys++;
            db_data_bytes += (k_slice.size() + v_slice.size());
        ...
        if (db_live_keys != live_keys_count_) {
            err_msg = "Live keys count mismatch: expected " + std::to_string(live_keys_count_) + ", got " + std::to_string(db_live_keys);
            return false;
        }
