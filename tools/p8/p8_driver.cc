...
#include "p8_config.h"
#include "p8_reference_model.h"
#include "p8_stats_collector.h"

using namespace study::p8;
...
class P8Driver {
public:
    P8Driver(const P8Config& config)
        : config_(config),
    ...
    bool LoadTrace() {
        ...
        if (!file.is_open()) {
            std::cerr << "Failed to open P8 trace: " << config_.trace_path << std::endl;
            return false;
        }
        ...
        std::cout << "[P8Driver] Loaded " << trace_ops_.size() << " deterministic ops from " << config_.trace_path << "\n";
        return true;
    }
    ...
    bool PreloadDatabase() {
        ...
        for (uint64_t k = 0; k < config_.total_keys; ++k) {
            std::string key = P8ReferenceModel::FormatKey(k);
            std::string val = P8ReferenceModel::GenerateValue(k, 1, config_.value_size);
    ...
    void RunExperiment() {
        std::cout << "\n=========================================================\n";
        std::cout << "Starting P8 Flush-Oracle Experiment: " << config_.exp_id << " (" << config_.group_name << ")\n";
        std::cout << "DeleteRange Ratio: " << (config_.del_range_ratio * 100.0)
                  << "%, Oracle Flush: " << (config_.enable_oracle_flush ? "ENABLED" : "DISABLED")
                  << (config_.enable_oracle_flush ? (" (at " + std::to_string(config_.flush_del_range_milestone) + " del ranges)") : "") << "\n";
        std::cout << "=========================================================\n";
        ...
        flush_triggered_.store(false);
        next_progress_milestone_.store(config_.total_ops / 100);

        auto exp_start_time = std::chrono::high_resolution_clock::now();

        std::chrono::high_resolution_clock::time_point phase_a_start = exp_start_time;
        std::chrono::high_resolution_clock::time_point phase_a_end;
        std::chrono::high_resolution_clock::time_point phase_b_start;
        std::chrono::high_resolution_clock::time_point phase_b_end;
        std::chrono::high_resolution_clock::time_point phase_c_start;
        std::chrono::high_resolution_clock::time_point phase_c_end;

        std::atomic<bool> phase_a_recorded(false);
        std::atomic<bool> phase_b_recorded(false);
        ...
        // Oracle Flush Controller Thread
        std::thread flush_thread;
        if (config_.enable_oracle_flush) {
            flush_thread = std::thread([&]() {
                while (!stop_requested_.load()) {
                    if (cumulative_del_ranges_.load() >= config_.flush_del_range_milestone && !flush_triggered_.exchange(true)) {
                        auto t_start = std::chrono::high_resolution_clock::now();
                        double start_sec = std::chrono::duration_cast<std::chrono::duration<double>>(t_start - exp_start_time).count();

                        std::cout << "\n[Oracle Controller] Triggering forced Flush() at cumulative del ranges = "
                                  << cumulative_del_ranges_.load() << " (t=" << std::fixed << std::setprecision(3) << start_sec << "s) ...\n";
                        ...
                        double end_sec = std::chrono::duration_cast<std::chrono::duration<double>>(t_end - exp_start_time).count();
                        double dur_ms = (end_sec - start_sec) * 1000.0;

                        std::cout << "[Oracle Controller] Flush() completed in " << std::fixed << std::setprecision(2)
                                  << dur_ms << " ms. Status: " << fs.ToString() << "\n\n";

                        stats_.SetFlushTiming(start_sec, end_sec);
                        break;
                    }
        ...
        for (int t = 0; t < config_.num_threads; ++t) {
            workers.emplace_back([&, t]() {
                ...
                while (true) {
                    ...
                    // Determine Phase:
                    // Phase 0 (A): 0 ~ 45% (0 ~ 90,000)
                    // Phase 1 (B): 45% ~ 55% (90,001 ~ 110,000)
                    // Phase 2 (C): 55% ~ 100% (110,001 ~ 200,000)
                    int phase = (idx < 90000) ? 0 : ((idx < 110000) ? 1 : 2);

                    if (idx >= 90000 && !phase_a_recorded.exchange(true)) {
                        phase_a_end = std::chrono::high_resolution_clock::now();
                        phase_b_start = phase_a_end;
                    }
                    if (idx >= 110000 && !phase_b_recorded.exchange(true)) {
                        phase_b_end = std::chrono::high_resolution_clock::now();
                    ...
                    if (op.op_type == 0) {
                        ...
                        std::string begin_key = P8ReferenceModel::FormatKey(begin_k);
                        std::string end_key = P8ReferenceModel::FormatKey(end_k);
                        ...
                        auto t1 = std::chrono::high_resolution_clock::now();
                        uint64_t lat_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
                        stats_.RecordScan(phase, lat_ns, span_req, returned_keys);

                    } else if (op.op_type == 2) {
                        // Put
                        std::string key_str = P8ReferenceModel::FormatKey(op.key1);
                        std::string val_str = P8ReferenceModel::GenerateValue(op.key1, 2, config_.value_size);
                        auto t0 = std::chrono::high_resolution_clock::now();
                        rocksdb::Status s = db_->Put(write_opts, key_str, val_str);
                        auto t1 = std::chrono::high_resolution_clock::now();
                        uint64_t lat_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count();
                        if (s.ok()) {
                            ref_model_.ApplyPut(op.key1);
                            stats_.RecordPut(phase, lat_ns);
                        }

                    } else if (op.op_type == 3) {
                        // DeleteRange
                        std::string b_k = P8ReferenceModel::FormatKey(op.key1);
                        std::string e_k = P8ReferenceModel::FormatKey(op.key2);
                        auto t0 = std::chrono::high_resolution_clock::now();
                        ...
                        if (s.ok()) {
                            ...
                            stats_.RecordDeleteRange(phase, lat_ns);
        ...
        phase_c_end = exp_end_time;
        double total_elapsed_sec = std::chrono::duration_cast<std::chrono::duration<double>>(exp_end_time - exp_start_time).count();

        double phase_a_dur = std::chrono::duration_cast<std::chrono::duration<double>>(phase_a_end - phase_a_start).count();
        double phase_b_dur = std::chrono::duration_cast<std::chrono::duration<double>>(phase_b_end - phase_b_start).count();
        double phase_c_dur = std::chrono::duration_cast<std::chrono::duration<double>>(phase_c_end - phase_c_start).count();

        stop_requested_.store(true);
        if (flush_thread.joinable()) flush_thread.join();
        if (wallclock_thread.joinable()) wallclock_thread.join();

        // Phase D Cooldown observation (10 seconds)
        ...
        stats_.AppendSummaryCsv(config_.summary_csv, config_.exp_id, config_.group_name, config_.desc,
                                total_elapsed_sec, phase_a_dur, phase_b_dur, phase_c_dur,
                                db_.get(), db_stats_,
...
private:
    ...
    P8ReferenceModel ref_model_;
    P8StatsCollector stats_;
    std::vector<P8Op> trace_ops_;
    std::atomic<bool> stop_requested_;
    std::atomic<uint64_t> completed_ops_;
    std::atomic<uint64_t> cumulative_del_ranges_;
    std::atomic<bool> flush_triggered_;
