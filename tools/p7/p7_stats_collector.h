...
struct P7SnapshotPoint {
    int index; // sec_idx or progress_pct
    double elapsed_sec;
...
class P7LatencyTracker {
public:
    P7LatencyTracker() : count_(0), sum_ns_(0), max_ns_(0) {
...
public:
    ...
    void CaptureWallClockSecond(int sec_idx, double elapsed_sec, uint64_t completed_ops, double prog_pct,
        ...
        P7SnapshotPoint pt = CollectSnapshot(sec_idx, elapsed_sec, completed_ops, prog_pct,
    ...
    void CaptureProgressSnapshot(int prog_pct, double elapsed_sec, uint64_t completed_ops,
                                 rocksdb::DB* db, std::shared_ptr<rocksdb::Statistics> stats,
                                 const std::string& db_path) {
        P7SnapshotPoint pt = CollectSnapshot(prog_pct, elapsed_sec, completed_ops, static_cast<double>(prog_pct),
...
private:
    P7SnapshotPoint CollectSnapshot(int index, double elapsed_sec, uint64_t completed_ops, double prog_pct,
                                   P7LatencyTracker& ctrl_lat, P7LatencyTracker& aff_lat, P7LatencyTracker& del_lat,
                                   P7LatencyTracker& scan_lat, std::atomic<uint64_t>& scan_keys,
                                   P7LatencyTracker& put_lat, P7LatencyTracker& del_range_lat,
    ...
    void DumpSnapshotVector(const std::string& filepath, const std::string& exp_id, const std::string& group_name,
                            const std::vector<P7SnapshotPoint>& records, const std::string& index_col_name) {
...
private:
    ...
    P7LatencyTracker main_put_lat_;
    P7LatencyTracker main_get_aff_lat_;
    P7LatencyTracker main_get_del_lat_;
    P7LatencyTracker main_get_ctrl_lat_;
    P7LatencyTracker main_scan_lat_;
    P7LatencyTracker main_del_range_lat_;

    // Wall-clock interval trackers
    P7LatencyTracker sec_put_lat_, sec_get_aff_lat_, sec_get_del_lat_, sec_get_ctrl_lat_, sec_scan_lat_, sec_del_range_lat_;
    ...
    std::vector<P7SnapshotPoint> wallclock_records_;

    // Progress interval trackers
    P7LatencyTracker prog_put_lat_, prog_get_aff_lat_, prog_get_del_lat_, prog_get_ctrl_lat_, prog_scan_lat_, prog_del_range_lat_;
    ...
    std::vector<P7SnapshotPoint> progress_records_;
};
