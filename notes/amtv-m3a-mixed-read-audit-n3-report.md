...
- **AMTV-T0 表现**：物化次数严格为 **0**，物化耗时严格为 **0.00 ms**，锁争用严格为 **0**，锁等待严格为 **0.00 ms**！GetLive P50 保持在 **9.49 μs**，P99 仅为 **27.30 μs**（相比 Native-T0 延迟下降 **99.78%**，即加速 **447 倍**）。
- **机制机理**：证明 AMTV 基于 `AMTVSnapshot` 与分层跳表的点查旁路机制完全不受并发原生 Scan 的干扰。点查完全绕过了 `MemTable::ConstructFragmentedRangeTombstones()`，杜绝了读锁争用与分片列表重构。

### 问题 2：Scan-Intersect 在动态注入期的真实表现与排队连锁反应
**结论：Scan-Intersect 虽未走 AMTV 优化，但其锁争用被 AMTV 的点查旁路显著解救**。
- **数据事实**：
  - Native-T0 下：`Scan-Intersect` P50 为 4,473.43 μs，`reader_mutex` 争用达 3,120 次，锁等待达 13.65 线程秒；
  - AMTV-T0 下：`Scan-Intersect` P50 下降至 **1,777.33 μs**（提速 **2.52 倍**），锁争用降至 1,001 次，锁等待降至 1.50 线程秒。
- **排队缓解机理**：在 Native-T0 下，40,000 次 GetLive 与 10,000 次 Scan 全部争抢同一个 `reader_mutex`，50,000 个读操作发生群体踩踏。而在 AMTV-T0 下，由于 40,000 次 GetLive 彻底退出 `reader_mutex` 竞争队列，争用源只剩下 10,000 次 Scan 本身，锁等待时间骤降 **89%**。
- **未解的原生限制**：AMTV-T0 依然产生了 **3,995 次**原生墓碑物化与 **55,643 次**迭代器重寻（`scan_reseek_count`），且单次 Scan 仍需耗费 1.78 ms，这表明未经改造的原生 Scan 路径本身存在沉重的分片重构与重寻包袱。

### 问题 3：原生 Scan 路径与点查的耗时量级对比
**结论：Scan 耗时比点查高出 2 个数量级（187 倍），以 10% 的操作样本霸占了 88% 的总执行时间**。
...
- **时间占用量化**：
  - 10,000 次 Scan 累计占用线程耗时：$10,000 \times 1.84\ \text{ms} \approx \mathbf{18.4\ \text{线程秒}}$；
  - 其余 90,000 次前台操作（Get/Put/DeleteRange）累计占用：约 $\mathbf{2.5\ \text{线程秒}}$；
  - **结论**：Scan 在样本数仅占 10% 的情况下，消耗了 Phase B **超过 88% 的 CPU 线程时间**。原生 Scan 路径是混合负载下名副其实的系统耗时瓶颈。

### 问题 4：整体吞吐率与端到端稀释效应
**结论：从纯点查负载的近 100 倍加速，稀释至混合负载下的 11.4 倍加速**。
- **数据对比**：
  - M2d（纯 GetLive + Put + DeleteRange）：Phase B 吞吐从 Native-T0 的 ~3,300 IOPS 跃升至 AMTV-T0 的 ~330,000 IOPS（约 **100 倍**提升）；
  - M3a（引入 10% 原生 Scan）：Native-T0 为 3,275 IOPS，AMTV-T0 为 37,345 IOPS（提升 **11.4 倍**）。
- **稀释原因**：根据阿姆达尔定律（Amdahl's Law），由于 AMTV 尚未对 Scan 路径做任何优化，每个 Worker 线程在执行到 Scan 时均被原生物化与前向扫描阻塞 1.7 ms 以上，这成为不可消除的串行化底噪。
- **正面价值**：即便存在这一稀释，AMTV-T0 依然取得了 **11.4 倍**的吞吐提升，且将 Phase B 耗时从 **30.55 秒**大幅缩短至 **2.68 秒**，消除了点查踩踏导致的整机停滞。

### 问题 5：对推进 M1c Scan 架构改造的决策依据
**结论：数据提供了坚实且紧迫的量化支撑，明确批准推进 M1c Scan 架构设计**。
1. **点查优化已达极致，Scan 成为绝对首要瓶颈**：AMTV 的 GetOnly 机制在点查上已经无可挑剔（0 物化、0 排队、9 μs 稳定延迟）。系统若要进一步突破 3.7 万 IOPS 并向数十万 IOPS 迈进，唯一的拦路虎正是单次耗时 1.7 ms 的原生 Scan。
...
3. **M1c 的定位与价值**：M1c Scan 架构必须基于 AMTV 已经维护的严格多层有序墓碑切片，向迭代器提供免锁、免动态物化且单调前进的墓碑分片流，从而将 Scan 从 1.7 ms 压缩至微秒级。
